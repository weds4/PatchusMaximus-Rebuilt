...
module.exports = function(xelib, fh, patcherPath, patchFile, settings, helpers, locals){
	...
	function namingMimic(rec) {
		let itemName = xelib.HasElement(rec,`FULL`)? xelib.GetValue(rec,`FULL`).toPascalCase(): `<NOTEXT>`;
	...
	function initRefMaps(){
		let buildEDIDMap = (handle, sig) => xelib.BuildReferenceMap(handle, sig, xelib.EditorID);
		locals.skyrimKeywords = buildEDIDMap(locals.Skyrim_Master, keywordSig);
		locals.permaKeywords = buildEDIDMap(locals.PerkusMaximus_Master, keywordSig);
		locals.skyrimMisc = buildEDIDMap(locals.Skyrim_Master, miscSig);
		locals.permaMisc = buildEDIDMap(locals.PerkusMaximus_Master, miscSig);
		locals.dragonbornMisc = buildEDIDMap(locals.dragonborn_Master, miscSig);
		locals.skyrimPerks = buildEDIDMap(locals.Skyrim_Master, perkSig);
		locals.permaPerks = buildEDIDMap(locals.PerkusMaximus_Master, perkSig);
	}

	function initArmorPatcher() {
