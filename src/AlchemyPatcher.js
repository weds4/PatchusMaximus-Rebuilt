module.exports = function(xelib, Extensions, constants, patchFile, settings, helpers, locals){
			...
			filter: rec => {//Called for each loaded record. Return false to skip patching a record
				...
				return xelib.HasElement(rec, `Effects`)
					...
					&& Object.keys(potionEffects).every(effect => 
						getAlchemyEffect(potionEffects[effect].mgefHandle)!== null);
			}
		},
		patch: function (rec) {
	...
	const loadAndPatch_Ingredients = {
		load: {
			signature: `INGR`,
			filter: rec => {//Called for each loaded record. Return false to skip patching a record
				...
				return xelib.HasElement(rec, `Effects`)
					&& isIngrAllowed(rec) 
					&& Object.keys(ingrEffects).some(effect => getAlchemyEffect(ingrEffects[effect].mgefHandle))
			}
		},
		patch: function (rec) {
			if (settings.UseThief) {
