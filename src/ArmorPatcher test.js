...
function ArmorPatcher (patchFile, settings, helpers, locals) {
  let recordsPatcher = {
    records: (patchFile, settings, helpers, locals) => {
...
module.exports = (patchFile, settings, helpers, locals) => {
  let ArmorPatcher1 = {
    records: (filesToPatch, helpers, settings, locals) => {
      helpers.logMessage(`test`);
      //patch things that need to be used, but not themselves changed in the patch
      if (locals.UseWarrior) {
        helpers.logMessage(`Getting clothes`);
        let clothes = xelib.GetRecords(filesToPatch, `ARMO`)//meltdown recipies for clothes
  ...
  return [ArmorPatcher1, ArmorPatcher2]
};